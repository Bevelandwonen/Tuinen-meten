    ...
    for i in df_eenheid_perceel_filtered["Perceelnummer"].unique():
        if i in numbers_list:
            ...
            if df_eenheid_perceel_filtered_temp.shape[0] == 1:
                ...
                intersection = perceel_poly.intersection(gdf_weg_temp["geometry"])

                intersection_perceel_weg = intersection.area.sum()
                perceel_size = perceel_poly.area
                house_size = gdf2_temp.area.values[0]
                storage, storage_size = find_berging(perceel_poly, gdf_bgt_pand)
                garden_size = perceel_size - (intersection_perceel_weg + house_size + storage_size)
                #visualise_house_perceel(gdf_temp, perceel_poly, gdf2_temp, gdf_weg_temp, storage)
                #TODO: append sizes here
                df_eenheid_nieuw.loc[df_eenheid_nieuw["Pand Id"] == gdf2_temp["identificatie"].values[0], 'nieuw tuin opp'] = garden_size
            ...
            if df_eenheid_perceel_filtered_temp.shape[0] > 1:
                ...
                if aligned:
                    # We get perceel and iterate
                    one_row_housess = one_row_houses(gdf2_temp)

                    if one_row_housess:

                        for z, row in gdf2_temp.iterrows():
                            #print("---------------------------------House perceel --------------------------------------")
                            # Get matching walls with neighbours, Check of hoekhuis of niet.
                            matches = perceel_borders(gdf2_temp, row)

                            try:
                            # Hoekhuis
                                house = gdf2_temp[gdf2_temp["identificatie"] == row["identificatie"]]
                                if len(matches.keys()) == 1:
                                    lol = create_perceel_border(matches, perceel_poly)[0]
                                    new_poly = get_corner_house_perceel(lol, house, perceel_poly)

                                # Niet hoekhuis
                                if len(matches.keys()) == 2:
                                    new_perceel, new_poly = create_perceel(matches, perceel_poly)
                                ...
                                df_eenheid_nieuw.loc[df_eenheid_nieuw["Pand Id"] == row["identificatie"], 'nieuw tuin opp'] = "ERROR"
                                print("add")
                    else:
                        # Houses not in one row
                        ...
                        for z, row in gdf2_temp.iterrows():
                            ...
                            try:
                                if len(matches.keys()) == 1:
                                    x = matches.values()
                                    # Extract coordinates from the Point2D objects
                                    coordinates = [(point.x.evalf(), point.y.evalf()) for point in list(x)[0]]

                                    # Create the LineString
                                    outer_corner = False

                                    # Check if an house is an outer edge house.
                                    max_distance_between_houses = 0
                                    # Calculate the centroid for each polygon
                                    gdf2_temp['centroid'] = gdf2_temp.geometry.centroid
                                    # Calculate the distance between all centroids
                                    distance_matrix = gdf2_temp['centroid'].apply(lambda x: gdf2_temp['centroid'].distance(x))
                                    # Convert the distance matrix to a numpy array for easier manipulation
                                    distance_array = np.array(distance_matrix)
                                    # Find the indices of the maximum distance
                                    max_distance_indices = np.unravel_index(np.argmax(distance_array, axis=None), distance_array.shape)

                                    #polygon_indices = gdf.index[list(max_distance_indices)]
                                    if row["identificatie"] == list(gdf2_temp["identificatie"])[max_distance_indices[0]] or row["identificatie"] == list(gdf2_temp["identificatie"])[max_distance_indices[1]]:
                                        outer_corner = True
                                    #if true, apply old method
                                    if outer_corner:
                                        lol = create_perceel_border(matches, perceel_poly)[0]
                                        perceel = get_corner_house_perceel(lol, house, perceel_poly)
                                    else:
                                        perceel_border = create_perceel_border(matches, perceel_poly)[0]
                                        parallel_house_lines = find_parallel_edge(house, perceel_border)
                                        furthest_parallel_line = find_furthest_wall(parallel_house_lines, perceel_border)
                                        parallel_wall = list(furthest_parallel_line.coords)
                                        matches2 = {"1": [Point(parallel_wall[0]), Point(parallel_wall[1])]}
                                        perceel_border2 = create_perceel_border(matches2, perceel_poly)[0]

                                        line1_coords = list(perceel_border.coords)
                                        neighbour_lines_coords = list(perceel_border2.coords)

                                        # Determine the four corners of the rectangle
                                        corners = [
                                            line1_coords[0],
                                            line1_coords[1],
                                            neighbour_lines_coords[1],
                                            neighbour_lines_coords[0]
                                        ]

                                        # Create the polygon
                                        perceel = Polygon(corners)

                                    # Create a polygon using the four corners
                                    gdf_polygon1 = gpd.GeoDataFrame(geometry=[perceel])
                                    gdf_weg_temp = gdf_weg[gdf_weg.intersects(gdf_temp.union_all())]

                                    storage, storage_size = find_berging(gdf_polygon1, gdf_bgt_pand)
                                    garden_size = calc_areas(gdf_weg_temp, perceel, house, storage_size)
                                    visualise_house_perceel(gdf_temp, perceel, gdf2_temp, gdf_weg_temp, storage)
                                    df_eenheid_nieuw.loc[df_eenheid_nieuw["Pand Id"] == row["identificatie"], 'nieuw tuin opp'] = garden_size
                                    print(garden_size)
                                # Niet hoekhuis
                                if len(matches.keys()) == 2:
